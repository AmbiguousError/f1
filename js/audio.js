...
export function updateAudio(speed) {
    ...
    const isMini = cfg.carClass === 'mini'; const isRally = cfg.carClass === 'rally'; let gear, rpm, frequency;
    if (isMini) { gear = Math.min(6, Math.max(1, Math.ceil(kph / 30))); const gearMin = (gear - 1) * 30; const gearMax = gear * 30; rpm = (kph - gearMin) / (gearMax - gearMin); frequency = 30 + (rpm * 120) + (gear * 10); }
    else if (isRally) { gear = Math.min(7, Math.max(1, Math.ceil(kph / 35))); const gearMin = (gear - 1) * 35; const gearMax = gear * 35; rpm = (kph - gearMin) / (gearMax - gearMin); frequency = 40 + (rpm * 150) + (gear * 12); }
    else { gear = Math.min(9, Math.max(1, Math.ceil(kph / 38))); const gearMin = (gear - 1) * 38; const gearMax = gear * 38; rpm = (kph - gearMin) / (gearMax - gearMin); frequency = 50 + (rpm * 200) + (gear * 15); }
