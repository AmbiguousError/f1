...
export function generateCircuit() {
    ...
    state.trackPoints = state.trackCurve.getSpacedPoints(cfg.trackRes);
    const distFirstLast = state.trackPoints[0].distanceTo(state.trackPoints[state.trackPoints.length - 1]);
    if (distFirstLast < 1.0) state.trackPoints.pop();
    state.checkpoints = [
        state.trackPoints[0],
        state.trackPoints[Math.floor(state.trackPoints.length / 3)],
        state.trackPoints[Math.floor((state.trackPoints.length * 2) / 3)],
    ];
    ...
    const colors = [];
    const indices = [];
    const cAsphalt = new THREE.Color(surf.track);
    const cKerbRed = new THREE.Color(0xcc0000);
    const cKerbWhite = new THREE.Color(0xffffff);
    const cLine = new THREE.Color(0xffffff);
    ...
    for (let i = 0; i < len; i++) {
        ...
        const p2 = state.trackPoints[(i + 1) % len];
        const p3 = state.trackPoints[(i + 2) % len];
        ...
        for (let j = 0; j < offsets.length; j++) {
            ...
            if (j === 0 || j === 5) {
                ...
                col = seg % 2 === 0 ? cKerbRed : cKerbWhite;
            } else if (j === 1 || j === 4) {
                col = cLine;
            }
        ...
        const row = 6;
        const base = i * row;
        const nextBase = ((i + 1) % len) * row;
        for (let k = 0; k < 5; k++) {
            indices.push(base + k, base + k + 1, nextBase + k);
            indices.push(base + k + 1, nextBase + k + 1, nextBase + k);
        }
    }
    state.scene.add(sandTrapsMesh);
    ...
    const trackMat = new THREE.MeshStandardMaterial({
        vertexColors: true,
        roughness: trackRough,
        metalness: trackMetal,
        flatShading: false,
        side: THREE.DoubleSide,
    });
    const trackMesh = new THREE.Mesh(trackGeo, trackMat);
    trackMesh.receiveShadow = true;
    state.scene.add(trackMesh);

    const trimeshVerts = vertices;
    const trimeshIndices = indices;
    const trackShape = new CANNON.Trimesh(trimeshVerts, trimeshIndices);
    const trackBody = new CANNON.Body({ mass: 0, material: state.world.defaultMaterial });
    trackBody.addShape(trackShape);
    state.world.addBody(trackBody);
    const groundBody = new CANNON.Body({ mass: 0, material: state.world.defaultMaterial });
    ...
    // Start Line Marking
    const startPos = state.trackCurve.getPoint(0);
    const nextPos = state.trackCurve.getPoint(0.01);
    ...
    lineMesh.position.y = startPos.y + 0.06;
    lineMesh.lookAt(nextPos);
    state.scene.add(lineMesh);

    // Starting Grid Paint
    const gridSlotGroup = new THREE.Group();
    ...
    for (let i = 0; i < cfg.opponents; i++) {
        ...
        const pSlot = state.trackPoints[idx];
        const pNext = state.trackPoints[(idx + 1) % cfg.trackRes];
        ...
        const sCenter = pSlot.clone().add(sideSlot.clone().multiplyScalar(offset));

        const leftLine = new THREE.Mesh(new THREE.BoxGeometry(0.2, 0.02, 5.0), gridMat);
        leftLine.position.set(-1.25, 0, 0);
        const topLine = new THREE.Mesh(new THREE.BoxGeometry(2.5, 0.02, 0.2), gridMat);
        topLine.position.set(0, 0, 2.5);
        const rightLine = new THREE.Mesh(new THREE.BoxGeometry(0.2, 0.02, 5.0), gridMat);
        rightLine.position.set(1.25, 0, 0);

        const sGroup = new THREE.Group();
        sGroup.add(leftLine);
        sGroup.add(topLine);
        sGroup.add(rightLine);
        sGroup.position.copy(sCenter);
        sGroup.position.y += 0.055;
        sGroup.rotation.y = Math.atan2(tanSlot.x, tanSlot.z);
...
export function generatePitLane() {
    ...
    const bollardMat = new THREE.MeshStandardMaterial({ color: 0xff6600, roughness: 0.6 });
    const bollardGeo = new THREE.ConeGeometry(0.3, 0.8, 8);
    bollardGeo.translate(0, 0.4, 0);
    const seatMat = new THREE.MeshStandardMaterial({ color: 0x3498db, roughness: 0.9 });
    const structureMat = new THREE.MeshStandardMaterial({ color: 0x555555 });
    const standRoofMat = new THREE.MeshStandardMaterial({ color: 0xdde4e8, roughness: 0.5, metalness: 0.2 });

    for (let i = -pitLen; i <= pitLen; i++) {
        let idx = (((cfg.trackRes + i) % cfg.trackRes) + cfg.trackRes) % cfg.trackRes;
        let nextIdx = (((cfg.trackRes + i + 1) % cfg.trackRes) + cfg.trackRes) % cfg.trackRes;
        ...
        const currentIn = trackEdge;
        const currentOut = trackEdge + Math.max(0.1, t * (wallWidth + pitWidth));
        const vIn = p1.clone().add(side.clone().multiplyScalar(currentIn));
        const vOut = p1.clone().add(side.clone().multiplyScalar(currentOut));
        pitVertices.push(vIn.x, vIn.y + 0.05, vIn.z);
        pitVertices.push(vOut.x, vOut.y + 0.05, vOut.z);
        const uvY = (i + pitLen) / (pitLen * 2);
        pitUvs.push(0, uvY * 10);
        pitUvs.push(1, uvY * 10);
        ...
        if (i < pitLen) {
            ...
            if (segmentWidth > 0.5) {
                ...
                const floorBody = new CANNON.Body({ mass: 0, material: state.world.defaultMaterial });
                floorBody.addShape(floorShape);
                floorBody.position.copy(floorCenter);
                floorBody.position.y += 0.04;
                floorBody.quaternion.setFromEuler(0, Math.atan2(tan.x, tan.z), 0);
                state.world.addBody(floorBody);
            }
        }

        if (t > 0 && t < 1 && i % 3 === 0) {
            const bPos = p1.clone().add(side.clone().multiplyScalar(trackEdge + (currentOut - trackEdge) * 0.2));
            const bollard = new THREE.Mesh(bollardGeo, bollardMat);
            bollard.position.copy(bPos);
            bollard.position.y += 0.05;
            linesGroup.add(bollard);
        }
        if (t === 1) {
            ...
            if (i % 5 === 0 && i < pitLen - rampLen - 5) {
                const garageDepth = 8;
                const garageWidth = dist * 5;
                const garageCenter = p1.clone().add(side.clone().multiplyScalar(currentOut + garageDepth / 2 + 0.5));
                if (isSpaceClear(garageCenter, trackEdge + 3.0)) {
                    const gGroup = new THREE.Group();
                    gGroup.position.copy(garageCenter);
                    gGroup.rotation.y = Math.atan2(tan.x, tan.z);
                    const gMesh = new THREE.Mesh(new THREE.BoxGeometry(garageDepth, 5, garageWidth - 0.2), garageMat);
                    gMesh.position.y = 2.5;
                    gGroup.add(gMesh);
                    const roof = new THREE.Mesh(
                        new THREE.BoxGeometry(garageDepth + 1, 0.5, garageWidth + 0.5),
                        roofMat
                    );
                    roof.position.y = 5.25;
                    gGroup.add(roof);
                    const door = new THREE.Mesh(new THREE.PlaneGeometry(garageWidth - 1.5, 3.5), doorMat);
                    door.position.set(-garageDepth / 2 - 0.01, 1.75, 0);
                    door.rotation.y = -Math.PI / 2;
                    gGroup.add(door);
                    garageGroup.add(gGroup);
                }
            }
        }
        if (i === -pitLen + rampLen || i === pitLen - rampLen) {
            const lineCenter = p1.clone().add(side.clone().multiplyScalar((currentIn + currentOut) / 2));
            const lineGeo = new THREE.PlaneGeometry(currentOut - currentIn, 0.6);
            lineGeo.rotateX(-Math.PI / 2);
            const lineMat = new THREE.MeshBasicMaterial({ color: 0xffffff });
            const lineMesh = new THREE.Mesh(lineGeo, lineMat);
            lineMesh.position.copy(lineCenter);
            lineMesh.position.y += 0.055;
            lineMesh.rotation.y = Math.atan2(tan.x, tan.z);
        ...
        if (i === 0) {
            ...
            const boxCenter = p1.clone().add(side.clone().multiplyScalar(trackEdge + wallWidth + pitWidth / 2));
            state.pitBoxPosition = { x: boxCenter.x, y: boxCenter.y, z: boxCenter.z, radius: 15 };
        }

        // STADIUM GRANDSTANDS
        if (i % 8 === 0 && i > -pitLen + 5 && i < pitLen - 5) {
            const standWidth = dist * 8;
            const standDepth = 15;
            const standCenter = p1.clone().add(side.clone().multiplyScalar(-trackEdge - standDepth / 2 - 5));
            if (isSpaceClear(standCenter, standDepth / 2 + trackEdge + 5.0)) {
                const gsGroup = new THREE.Group();
                gsGroup.position.copy(standCenter);
                gsGroup.rotation.y = Math.atan2(tan.x, tan.z) + Math.PI;
                for (let s = 0; s < 4; s++) {
                    const tier = new THREE.Mesh(
                        new THREE.BoxGeometry(standDepth - s * 3, 1, standWidth - 0.5),
                        seatMat
                    );
                    tier.position.set(s * 1.5, s * 1 + 0.5, 0);
                    gsGroup.add(tier);
                }
                const roofGeo = new THREE.BoxGeometry(standDepth + 2, 0.5, standWidth);
                const roof = new THREE.Mesh(roofGeo, standRoofMat);
                roof.position.set(0, 8, 0);
                roof.rotation.z = -0.1;
                gsGroup.add(roof);
                const pillarGeo = new THREE.CylinderGeometry(0.2, 0.2, 8);
                const p1Mesh = new THREE.Mesh(pillarGeo, structureMat);
                p1Mesh.position.set(-6, 4, standWidth / 3);
                const p2Mesh = new THREE.Mesh(pillarGeo, structureMat);
                p2Mesh.position.set(-6, 4, -standWidth / 3);
                gsGroup.add(p1Mesh);
                gsGroup.add(p2Mesh);
    ...
    state.scene.add(garageGroup);
    state.scene.add(linesGroup);
    state.scene.add(grandstandGroup);
    const geo = new THREE.BufferGeometry();
    geo.setAttribute('position', new THREE.Float32BufferAttribute(pitVertices, 3));
    geo.setAttribute('uv', new THREE.Float32BufferAttribute(pitUvs, 2));
    geo.setIndex(pitIndices);
    geo.computeVertexNormals();
    const mat = new THREE.MeshStandardMaterial({ color: 0x444444, roughness: 0.95, side: THREE.DoubleSide });
...
export function generateScenery() {
    const treeCount = 800;
    const trunkGeo = new THREE.CylinderGeometry(0.6, 0.8, 3, 7);
    ...
    const leavesMat = new THREE.MeshStandardMaterial({
        color: cfg.time === 'sunset' ? 0x2d4c1e : 0x184a22,
        roughness: 0.8,
    });
    ...
    while (placed < treeCount && attempts < 5000) {
        attempts++;
        const x = (state.rng() - 0.5) * 900;
        const z = (state.rng() - 0.5) * 900;
        let tooClose = false;
        for (let i = 0; i < state.trackPoints.length; i += 4) {
            const dx = x - state.trackPoints[i].x;
            const dz = z - state.trackPoints[i].z;
            if (dx * dx + dz * dz < 1800) {
                tooClose = true;
                break;
            }
        ...
        if (!tooClose) {
            const scale = 0.7 + state.rng() * 0.6;
            dummy.position.set(x, 0, z);
            dummy.scale.set(scale, scale, scale);
            dummy.rotation.y = state.rng() * Math.PI * 2;
            dummy.updateMatrix();
    ...
    state.sceneryMeshes.push(trunkMesh, leavesMesh);
}
...
export function setupMinimap() {
    ...
    if (state.trackPoints.length > 0) {
        const p0 = toMap(state.trackPoints[0]);
        bgCtx.moveTo(p0.x, p0.y);
        for (let i = 1; i < state.trackPoints.length; i++) {
            const p = toMap(state.trackPoints[i]);
...
export function updateMinimap() {
    ...
    if (state.chassisBody) {
        const c = toMap(state.chassisBody.position);
...
function findClearRespawnIdx(startIdx, excludeBody) {
    ...
    state.aiCars.forEach((ai) => {
        if (ai.body !== excludeBody) others.push(ai.body);
    });
    const isClear = (idx) => {
        const tp = state.trackPoints[idx];
        return others.every((b) => {
            const dx = b.position.x - tp.x,
                dz = b.position.z - tp.z;
            return dx * dx + dz * dz > clearRadiusSq;
        });
    };
    if (isClear(startIdx)) return startIdx;
    for (let step = 5; step <= 60; step += 5) {
        const fwd = (startIdx + step) % state.trackPoints.length;
        if (isClear(fwd)) return fwd;
        const back = (startIdx - step + state.trackPoints.length) % state.trackPoints.length;
        if (isClear(back)) return back;
    }
...
export function teleportToTrack(body) {
    let minD = Infinity,
        closest = state.trackPoints[0],
        closestIdx = 0;
    const p = body.position;
    for (let i = 0; i < state.trackPoints.length; i += 10) {
        const tp = state.trackPoints[i];
        const dx = p.x - tp.x;
        const dy = p.y - tp.y;
        const dz = p.z - tp.z;
        ...
        if (d < minD) {
            minD = d;
            closest = state.trackPoints[i];
            closestIdx = i;
        }
    }
    closestIdx = findClearRespawnIdx(closestIdx, body);
    closest = state.trackPoints[closestIdx];
    const nextP = state.trackPoints[(closestIdx + 20) % state.trackPoints.length];
    body.position.set(closest.x, closest.y + 2, closest.z);
    body.velocity.set(0, 0, 0);
    body.angularVelocity.set(0, 0, 0);
    const dummy = new THREE.Object3D();
    dummy.position.copy(closest);
    dummy.lookAt(nextP);
