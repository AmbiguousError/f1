...
import { buildCarMesh, buildMiniMesh, buildRallyMesh, buildTyreStripe, wGeoFront, wGeoRear, wGeoMini, wMat, rimMat } from './car-models.js';
...
export function createAICar(startIdx, offset, color, id, name) {
    ...
    const mesh = isMini ? buildMiniMesh(color) : (isRally ? buildRallyMesh(color) : buildCarMesh(color)); state.scene.add(mesh); body.userData = { mesh: mesh };
    const vehicle = new CANNON.RaycastVehicle({ chassisBody: body, indexRightAxis: 0, indexUpAxis: 1, indexForwardAxis: 2 });
    const currentGrip = cfg.weather === 'wet' ? 2.0 : 5.0; const wRadiusF = isMini ? 0.3 : 0.45; const wRadiusR = isMini ? 0.3 : (isRally ? 0.45 : 0.6);
    const wOpts = { radius: wRadiusF, directionLocal: new CANNON.Vec3(0, -1, 0), suspensionStiffness: 50, suspensionRestLength: 0.5, frictionSlip: currentGrip, dampingRelaxation: 2.3, dampingCompression: 4.4, maxSuspensionForce: 100000, rollInfluence: 0.01, axleLocal: new CANNON.Vec3(1, 0, 0), chassisConnectionPointLocal: new CANNON.Vec3(1, 1, 0), maxSuspensionTravel: 0.25, useCustomSlidingRotationalSpeed: true, customSlidingRotationalSpeed: -30 };
    const fZ = isMini ? 0.9 : (isRally ? 1.05 : 1.85); const rZ = isMini ? -0.9 : (isRally ? -1.15 : -2.1); const width = isMini ? 0.7 : (isRally ? 0.85 : 1.1); const h = isMini ? 0.2 : (isRally ? 0.35 : 0.4);
    vehicle.addWheel({ ...wOpts, chassisConnectionPointLocal: new CANNON.Vec3(width, h, fZ) }); vehicle.addWheel({ ...wOpts, chassisConnectionPointLocal: new CANNON.Vec3(-width, h, fZ) });
    ...
    vehicle.wheelInfos.forEach((w, i) => {
        ...
        if (isMini) { tire = new THREE.Mesh(wGeoMini, wMatCar); rim = new THREE.Mesh(new THREE.CylinderGeometry(0.18, 0.18, 0.15, 16), rimMatCar); }
        else if (isRally) { tire = new THREE.Mesh(wGeoFront, wMatCar); rim = new THREE.Mesh(new THREE.CylinderGeometry(0.25, 0.25, 0.4, 16), rimMatCar); }
        else { tire = new THREE.Mesh(isRear ? wGeoRear : wGeoFront, wMatCar); rim = new THREE.Mesh(new THREE.CylinderGeometry(0.25, 0.25, isRear ? 0.7 : 0.4, 16), rimMatCar); }
        tire.rotation.z = Math.PI / 2; tire.castShadow = true; rim.rotation.z = Math.PI / 2; grp.add(tire); grp.add(rim);
        const stripe = buildTyreStripe(isMini, isRally ? false : isRear, startingCompound); if (i % 2 !== 0) stripe.position.x *= -1; grp.add(stripe); aiStripes.push(stripe);
        state.scene.add(grp); wheels.push(grp);
    });

    mesh.position.copy(body.position); mesh.quaternion.copy(body.quaternion);
    for (let i = 0; i < 4; i++) { vehicle.updateWheelTransform(i); const t = vehicle.wheelInfos[i].worldTransform; wheels[i].position.copy(t.position); wheels[i].quaternion.copy(t.quaternion); }

    let speedBase = 260, speedVar = 30, cornBase = 0.8, cornVar = 0.4;
    if (cfg.difficulty === 'easy') { speedBase = 230; speedVar = 30; cornBase = 0.6; cornVar = 0.3; } else if (cfg.difficulty === 'hard') { speedBase = 290; speedVar = 30; cornBase = 1.0; cornVar = 0.4; }
...
export function createF1Car(startIdx, flyingStart, offset = 0) {
    ...
    const mesh = isMini ? buildMiniMesh(cfg.teamColor) : (isRally ? buildRallyMesh(cfg.teamColor) : buildCarMesh(cfg.teamColor));
    ...
    state.vehicle = new CANNON.RaycastVehicle({ chassisBody: state.chassisBody, indexRightAxis: 0, indexUpAxis: 1, indexForwardAxis: 2 });
    const currentGrip = cfg.weather === 'wet' ? 2.0 : 5.0; const wRadiusF = isMini ? 0.3 : 0.45; const wRadiusR = isMini ? 0.3 : (isRally ? 0.45 : 0.6);
    const wOpts = { radius: wRadiusF, directionLocal: new CANNON.Vec3(0, -1, 0), suspensionStiffness: 50, suspensionRestLength: 0.5, frictionSlip: currentGrip, dampingRelaxation: 2.3, dampingCompression: 4.4, maxSuspensionForce: 100000, rollInfluence: 0.01, axleLocal: new CANNON.Vec3(1, 0, 0), chassisConnectionPointLocal: new CANNON.Vec3(1, 1, 0), maxSuspensionTravel: 0.25, useCustomSlidingRotationalSpeed: true, customSlidingRotationalSpeed: -30 };
    const fZ = isMini ? 0.9 : (isRally ? 1.05 : 1.85); const rZ = isMini ? -0.9 : (isRally ? -1.15 : -2.1); const width = isMini ? 0.7 : (isRally ? 0.85 : 1.1); const h = isMini ? 0.2 : (isRally ? 0.35 : 0.4);
    state.vehicle.addWheel({ ...wOpts, chassisConnectionPointLocal: new CANNON.Vec3(width, h, fZ) }); state.vehicle.addWheel({ ...wOpts, chassisConnectionPointLocal: new CANNON.Vec3(-width, h, fZ) });
    ...
    state.vehicle.wheelInfos.forEach((w, i) => {
        ...
        tire.rotation.z = Math.PI / 2; tire.castShadow = true; rim.rotation.z = Math.PI / 2; grp.add(tire); grp.add(rim);
        const stripe = buildTyreStripe(isMini, isRally ? false : isRear, state.tyreCompoundIdx); if (i % 2 !== 0) stripe.position.x *= -1; grp.add(stripe); state.playerTyreStripes.push(stripe);
        state.scene.add(grp); state.visualWheels.push(grp);
    });

    mesh.position.copy(state.chassisBody.position); mesh.quaternion.copy(state.chassisBody.quaternion);
