...
const rallyBodyGeo = new THREE.BoxGeometry(1.4, 0.6, 2.7); const rallyCabinGeo = new THREE.BoxGeometry(1.2, 0.5, 1.5); const rallyWingGeo = new THREE.BoxGeometry(1.4, 0.08, 0.4); const rallyWingStrutGeo = new THREE.BoxGeometry(0.08, 0.25, 0.1); const rallyLightPodGeo = new THREE.BoxGeometry(0.6, 0.18, 0.25); const rallySpotlightGeo = new THREE.CylinderGeometry(0.12, 0.12, 0.15, 16); rallySpotlightGeo.rotateX(Math.PI / 2); const mudFlapGeo = new THREE.BoxGeometry(0.3, 0.45, 0.04);

// Register static global shapes to protection sets to prevent disposal in cleanup()
[bodyGeo, noseGeo, fwGeo, podGeo, rwGeo, rwTopGeo, haloGeo, helmGeo, wGeoFront, wGeoRear, wGeoMini, miniBodyGeo, miniRoofGeo, miniCabinGeo, miniLightGeo, rallyBodyGeo, rallyCabinGeo, rallyWingGeo, rallyWingStrutGeo, rallyLightPodGeo, rallySpotlightGeo, mudFlapGeo].forEach(g => globalGeometries.add(g));
...
export function buildRallyMesh(color) {
    const group = new THREE.Group(); const mainMat = new THREE.MeshStandardMaterial({ color: color, metalness: 0.2, roughness: 0.3 }); const blackMat = new THREE.MeshStandardMaterial({ color: 0x111111, roughness: 0.7 });
    const glassMat = new THREE.MeshStandardMaterial({ color: 0x111111, roughness: 0.1, metalness: 0.9 }); const chromeMat = new THREE.MeshStandardMaterial({ color: 0xdddddd, metalness: 0.8, roughness: 0.2 });
    const body = new THREE.Mesh(rallyBodyGeo, mainMat); body.position.set(0, 0.4, 0); group.add(body);
    const cabin = new THREE.Mesh(rallyCabinGeo, glassMat); cabin.position.set(0, 0.85, -0.15); group.add(cabin);
    const roofGeo = new THREE.BoxGeometry(1.15, 0.08, 1.4); roofGeo.translate(0, 1.1, -0.15); const roof = new THREE.Mesh(roofGeo, mainMat); group.add(roof); globalGeometries.add(roofGeo);
    const wing = new THREE.Mesh(rallyWingGeo, mainMat); wing.position.set(0, 1.15, -1.2); group.add(wing);
    const strutL = new THREE.Mesh(rallyWingStrutGeo, blackMat); strutL.position.set(0.4, 1.0, -1.25); group.add(strutL); const strutR = new THREE.Mesh(rallyWingStrutGeo, blackMat); strutR.position.set(-0.4, 1.0, -1.25); group.add(strutR);
    const pod = new THREE.Mesh(rallyLightPodGeo, blackMat); pod.position.set(0, 0.6, 1.3); group.add(pod);
    const bulbMat = new THREE.MeshBasicMaterial({ color: 0xffffff });
    for (let i = 0; i < 4; i++) {
        const xOffset = -0.375 + i * 0.25; const lightShell = new THREE.Mesh(rallySpotlightGeo, chromeMat); lightShell.position.set(xOffset, 0.62, 1.4); group.add(lightShell);
        const bulbGeo = new THREE.CylinderGeometry(0.1, 0.1, 0.02, 16); bulbGeo.rotateX(Math.PI / 2); const bulb = new THREE.Mesh(bulbGeo, bulbMat); bulb.position.set(xOffset, 0.62, 1.48); group.add(bulb); globalGeometries.add(bulbGeo);
    }
    const flapFL = new THREE.Mesh(mudFlapGeo, blackMat); flapFL.position.set(0.8, 0.25, -0.85); group.add(flapFL);
    const flapFR = new THREE.Mesh(mudFlapGeo, blackMat); flapFR.position.set(-0.8, 0.25, -0.85); group.add(flapFR);
    const flapRL = new THREE.Mesh(mudFlapGeo, blackMat); flapRL.position.set(0.8, 0.25, -2.15); group.add(flapRL);
    const flapRR = new THREE.Mesh(mudFlapGeo, blackMat); flapRR.position.set(-0.8, 0.25, -2.15); group.add(flapRR);
    const tailMat = new THREE.MeshBasicMaterial({ color: 0x440000 }); const tailGeo = new THREE.BoxGeometry(0.25, 0.12, 0.05);
    const tailL = new THREE.Mesh(tailGeo, tailMat); tailL.position.set(0.5, 0.5, -1.36); const tailR = new THREE.Mesh(tailGeo, tailMat); tailR.position.set(-0.5, 0.5, -1.36); group.add(tailL); group.add(tailR); globalGeometries.add(tailGeo);
    group.castShadow = true; group.traverse(c => c.castShadow = true); group.userData = { tailMat: tailMat }; return group;
}
...
export function buildTyreStripe(isMini, isRearOrRally, startCompoundIdx) {
    ...
    const offset = isMini ? 0.12 : (isRearOrRally ? 0.4 : 0.28); mesh.position.set(offset, 0, 0); return mesh;
}
